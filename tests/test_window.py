...
class TestWindowFull(unittest.TestCase):
    ...
    def test_update_side_limit_fillers(self):
        """Teste update_side_limit_fillers sans erreur."""
        try:
            self.window.update_side_limit_fillers(speed=5)
        except Exception as e:
            self.fail(f"update_side_limit_fillers raised an exception {e}")
    ...
    def test_update_side_limit_fillers_dx_reset(self):
        ...
        self.window.update_side_limit_fillers(speed=5)  # dx - 5 <= 0 déclenche la branche
